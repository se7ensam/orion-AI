...
```bash
# From services/downloader directory
...
### Method 3: Via Python CLI (Recommended)
...
# Custom download directory
python -m src.cli download --start-year 2009 --end-year 2010 --download-dir ./custom_data
```

### Method 4: Using Convenience Script

From the project root:

```bash
./orion download --start-year 2009 --end-year 2010 --max-workers 20
```

### Method 5: Docker

```bash
# Build Docker image (includes service build)
./docker-orchestrator.sh build

# Run download
./docker-orchestrator.sh download 2009 2010 20
```
